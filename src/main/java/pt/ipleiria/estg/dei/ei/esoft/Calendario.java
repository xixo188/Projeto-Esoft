...
public class Calendario {
    ...
    private static void showFaseGrupos(TorneioApp app, Store store) {
        ...
        for (int i = 0; i < numGrupos; i++) {
            ...
            for (int j = startIndex; j < endIndex; j++) {
                Equipa t = store.teams.get(j);
                int pts = 0, v = 0, e = 0, d = 0;

                for (Jogo g : store.games) {
                    if (g.phase.equals("Fase de Grupos") && g.state == EstadoJogo.CONCLUIDO) {
        ...
        List<Jogo> faseGruposJogos = store.games.stream().filter(g -> g.phase.equals("Fase de Grupos")).collect(Collectors.toList());

        for (Jogo g : faseGruposJogos) {
            String textoAcao = "Finalizado";
            if (g.state == EstadoJogo.AGENDADO) textoAcao = "Começar Jogo";
            else if (g.state == EstadoJogo.EM_CURSO) textoAcao = "Gerir Jogo";
        ...
        tabelaJogos.addMouseListener(new java.awt.event.MouseAdapter() {
            ...
            public void mouseClicked(java.awt.event.MouseEvent e) {
                ...
                if (row >= 0 && row < faseGruposJogos.size()) {
                    Jogo g = faseGruposJogos.get(row);

                    // Alterar a data
                    if (col == 0) {
                        if (g.state == EstadoJogo.AGENDADO) {
                    ...
                    if (col == 4) {
                        ...
                        } else if (g.state == EstadoJogo.EM_CURSO) {
                            app.showGameDetails(g);
                        } else if (g.state == EstadoJogo.CONCLUIDO) {
                            app.showGameDetails(g);
                        }
                    }
                }
        ...
        boolean torneioIniciado = store.games.stream().anyMatch(g -> g.state == EstadoJogo.EM_CURSO || g.state == EstadoJogo.CONCLUIDO);
    ...
    public static void showFaseEliminacao(TorneioApp app, Store store) {
        ...
        List<Jogo> quartos = store.games.stream().filter(g -> g.phase.equals("Quartos de Final")).collect(Collectors.toList());
        List<Jogo> semis = store.games.stream().filter(g -> g.phase.equals("Semifinais")).collect(Collectors.toList());
        List<Jogo> finais = store.games.stream().filter(g -> g.phase.equals("Final")).collect(Collectors.toList());
    ...
    private static JPanel criarColunaFase(TorneioApp app, Store store, String titulo, List<Jogo> jogos, int totalEsperado) {
        ...
        for (int i = 0; i < totalEsperado; i++) {
            ...
            Jogo g = (i < jogos.size()) ? jogos.get(i) : null;
    ...
    private static JPanel criarCartaoJogo(TorneioApp app, Store store, Jogo game) {
        ...
        if (game == null) {
            JLabel pendingLabel = new JLabel("A determinar", SwingConstants.CENTER);
            pendingLabel.setForeground(new Color(225, 225, 225));
            pendingLabel.setFont(new Font("Arial", Font.BOLD, 16));
            card.add(pendingLabel, BorderLayout.CENTER);
            return card;
        }
        ...
        JLabel dateLabel = textoBranco(game.dateTime);
        dateLabel.setFont(new Font("Arial", Font.PLAIN, 11));

        JLabel stateLabel = new JLabel(formatGameState(game), SwingConstants.CENTER);
        stateLabel.setOpaque(true);
        stateLabel.setForeground(Color.WHITE);
        stateLabel.setFont(new Font("Arial", Font.BOLD, 11));
        stateLabel.setBorder(BorderFactory.createEmptyBorder(3, 8, 3, 8));

        if (game.state == EstadoJogo.CONCLUIDO) {
            if (!"Fase de Grupos".equals(game.phase) && game.goalsA == game.goalsB) {
                stateLabel.setText("Empate inválido");
                stateLabel.setBackground(new Color(190, 55, 55));
            } else {
                stateLabel.setBackground(new Color(40, 130, 75));
            }
        } else if (game.state == EstadoJogo.EM_CURSO) {
            stateLabel.setBackground(new Color(210, 135, 35));
        } else {
            stateLabel.setBackground(new Color(65, 105, 135));
        }

        JLabel resultLabel = textoBranco(game.resultText());
        resultLabel.setHorizontalAlignment(SwingConstants.RIGHT);
        resultLabel.setFont(new Font("Arial", Font.BOLD, 18));

        header.add(dateLabel, BorderLayout.WEST);
        header.add(stateLabel, BorderLayout.CENTER);
        header.add(resultLabel, BorderLayout.EAST);

        JPanel teamsPanel = new JPanel();
        teamsPanel.setOpaque(false);
        teamsPanel.setLayout(new BoxLayout(teamsPanel, BoxLayout.Y_AXIS));

        teamsPanel.add(createTeamMatchLine(store, game, game.teamA, game.goalsA, game.goalsA > game.goalsB));
        teamsPanel.add(Box.createVerticalStrut(6));
        teamsPanel.add(new JSeparator());
        teamsPanel.add(Box.createVerticalStrut(6));
        teamsPanel.add(createTeamMatchLine(store, game, game.teamB, game.goalsB, game.goalsB > game.goalsA));

        JPanel footer = new JPanel(new BorderLayout());
        footer.setOpaque(false);

        JLabel stadiumLabel = textoBranco(game.stadium == null ? "Estádio não definido" : game.stadium.nome);
        stadiumLabel.setForeground(new Color(210, 210, 210));
        stadiumLabel.setFont(new Font("Arial", Font.PLAIN, 11));

        String actionText;
        if (game.state == EstadoJogo.AGENDADO) {
            actionText = "Clique para começar";
        } else if (game.state == EstadoJogo.EM_CURSO) {
            actionText = "Clique para gerir";
        } else {
            actionText = "Jogo terminado";
        }

        JLabel actionLabel = textoBranco(actionText);
        actionLabel.setForeground(new Color(210, 210, 210));
        actionLabel.setFont(new Font("Arial", Font.ITALIC, 11));

        footer.add(stadiumLabel, BorderLayout.WEST);
        footer.add(actionLabel, BorderLayout.EAST);

        card.add(header, BorderLayout.NORTH);
        card.add(teamsPanel, BorderLayout.CENTER);
        card.add(footer, BorderLayout.SOUTH);

        boolean participantsDefined = !"A determinar".equals(game.teamA) && !"A determinar".equals(game.teamB);
        ...
        if (participantsDefined) {
            ...
            card.addMouseListener(new java.awt.event.MouseAdapter() {
                ...
                public void mouseClicked(java.awt.event.MouseEvent event) {
                    if (game.state == EstadoJogo.AGENDADO) {
                        int option = JOptionPane.showConfirmDialog(app, "Deseja dar o apito inicial?", "Começar Jogo", JOptionPane.YES_NO_OPTION);
                        if (option == JOptionPane.YES_OPTION) {
                            game.state = EstadoJogo.EM_CURSO;
                            app.info("Jogo iniciado!");
                            app.showGameDetails(game);
                        }
                    } else if (game.state == EstadoJogo.EM_CURSO) {
                        app.showGameDetails(game);
                    } else if (game.state == EstadoJogo.CONCLUIDO) {
                        app.showGameDetails(game);
                    }
                }
            });
        } else {
            card.setToolTipText("Os participantes deste jogo ainda não estão definidos.");
        }

        return card;
    }

    private static JPanel createTeamMatchLine(Store store, Jogo game, String teamName, int goals, boolean winner) {
        JPanel panel = new JPanel(new BorderLayout(8, 2));
        panel.setOpaque(false);

        JLabel teamLabel = textoBranco(winner ? "✓ " + teamName : teamName);
        teamLabel.setFont(new Font("Arial", Font.BOLD, 14));

        if (winner) {
            teamLabel.setForeground(new Color(125, 225, 150));
        }

        JLabel scoreLabel = textoBranco(String.valueOf(goals));
        scoreLabel.setFont(new Font("Arial", Font.BOLD, 15));

        JLabel scorersLabel = new JLabel("Marcadores: " + getScorersText(store, game, teamName, goals));
        scorersLabel.setForeground(new Color(210, 210, 210));
        scorersLabel.setFont(new Font("Arial", Font.PLAIN, 11));

        panel.add(teamLabel, BorderLayout.WEST);
        panel.add(scoreLabel, BorderLayout.EAST);
        panel.add(scorersLabel, BorderLayout.SOUTH);

        return panel;
    }

    private static String getScorersText(Store store, Jogo game, String teamName, int goals) {
        List<String> scorers = store.findEventsByGame(game).stream()
                .filter(event -> event.type == TipoEventoJogo.GOLO)
                .filter(event -> event.team != null && event.team.name.equalsIgnoreCase(teamName))
                .map(event -> event.minute + "' " + event.playerName())
    ...
    private static String formatGameState(Jogo game) {
        if (game.state == EstadoJogo.CONCLUIDO) return "Concluído";
        if (game.state == EstadoJogo.EM_CURSO) return "Em curso";
        if (game.state == EstadoJogo.AGENDADO) return "Agendado";
        if (game.state == EstadoJogo.CANCELADO) return "Cancelado";
        return "Por agendar";
    }
    ...
    private static boolean apurarFaseEliminacao(TorneioApp app, Store store) {
        ...
        Equipa firstA = groupA.get(0);
        Equipa secondA = groupA.get(1);

        Equipa firstB = groupB.get(0);
        Equipa secondB = groupB.get(1);

        Equipa firstC = groupC.get(0);
        Equipa secondC = groupC.get(1);
    ...
    public static void atualizarEliminatorias(TorneioApp app, Store store) {
        List<Jogo> quarters = getGamesByPhase(store, "Quartos de Final");
        List<Jogo> semifinals = getGamesByPhase(store, "Semifinais");
        List<Jogo> finals = getGamesByPhase(store, "Final");

        if (quarters.size() == 4 && semifinals.size() == 2 && quarters.stream().allMatch(game -> game.state == EstadoJogo.CONCLUIDO)) {
            Equipa winnerQ1 = getWinner(store, quarters.get(0));
            Equipa winnerQ2 = getWinner(store, quarters.get(1));
            Equipa winnerQ3 = getWinner(store, quarters.get(2));
            Equipa winnerQ4 = getWinner(store, quarters.get(3));

            if (winnerQ1 != null && winnerQ2 != null && winnerQ3 != null && winnerQ4 != null) {
                Jogo semifinal1 = semifinals.get(0);
                Jogo semifinal2 = semifinals.get(1);

                if ("A determinar".equals(semifinal1.teamA) && "A determinar".equals(semifinal1.teamB)) {
                    semifinal1.teamA = winnerQ1.name;
                    semifinal1.teamB = winnerQ3.name;
                }

                if ("A determinar".equals(semifinal2.teamA) && "A determinar".equals(semifinal2.teamB)) {
                    semifinal2.teamA = winnerQ2.name;
                    semifinal2.teamB = winnerQ4.name;
                }
            }
        }

        if (semifinals.size() == 2 && finals.size() == 1 && semifinals.stream().allMatch(game -> game.state == EstadoJogo.CONCLUIDO)) {
            Equipa winnerS1 = getWinner(store, semifinals.get(0));
            Equipa winnerS2 = getWinner(store, semifinals.get(1));

            if (winnerS1 != null && winnerS2 != null) {
                Jogo finalGame = finals.get(0);

                if ("A determinar".equals(finalGame.teamA) && "A determinar".equals(finalGame.teamB)) {
                    finalGame.teamA = winnerS1.name;
                    finalGame.teamB = winnerS2.name;
                }
            }
        }

        if (finals.size() == 1 && finals.get(0).state == EstadoJogo.CONCLUIDO) {
            Equipa champion = getWinner(store, finals.get(0));

            if (champion != null && store.tournament != null) {
                store.tournament.state = "concluído";
            }
        }
    }

    private static List<Equipa> obterClassificacaoGrupo(Store store, int groupIndex) {
        ...
        for (int i = startIndex; i < endIndex; i++) {
            Equipa team = store.teams.get(i);
            ClassificationEntry entry = new ClassificationEntry(team);

            for (Jogo game : store.games) {
                if (!"Fase de Grupos".equals(game.phase) || game.state != EstadoJogo.CONCLUIDO) {
                    continue;
                }
        ...
        entries.sort(
                Comparator.comparingInt((ClassificationEntry entry) -> entry.points).reversed()
                        .thenComparing(Comparator.comparingInt((ClassificationEntry entry) -> entry.goalDifference()).reversed())
                        .thenComparing(Comparator.comparingInt((ClassificationEntry entry) -> entry.goalsFor).reversed())
                        .thenComparing(entry -> entry.team.name)
        );

        return entries.stream().map(entry -> entry.team).collect(Collectors.toList());
    }

    private static boolean todosJogosFaseConcluidos(Store store, String phase) {
        List<Jogo> games = getGamesByPhase(store, phase);
        return !games.isEmpty() && games.stream().allMatch(game -> game.state == EstadoJogo.CONCLUIDO);
    }

    private static boolean existeFase(Store store, String phase) {
        return store.games.stream().anyMatch(game -> phase.equals(game.phase));
    }

    private static List<Jogo> getGamesByPhase(Store store, String phase) {
        return store.games.stream().filter(game -> phase.equals(game.phase)).collect(Collectors.toList());
    }

    private static Equipa getWinner(Store store, Jogo game) {
        if (game == null || game.state != EstadoJogo.CONCLUIDO || game.goalsA == game.goalsB) {
            return null;
        }
        String winnerName = game.goalsA > game.goalsB ? game.teamA : game.teamB;
        return store.findTeamByName(winnerName);
    }

    private static void createPlaceholderKnockoutGame(Store store, String phase, LocalDateTime gameDate, int stadiumIndex) {
        Estadio stadium = store.stadiums.get(stadiumIndex % store.stadiums.size());
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm");

        Jogo game = new Jogo(
                store.nextId(), phase, "A determinar", "A determinar", gameDate.format(formatter), stadium
        );
        game.state = EstadoJogo.AGENDADO;
        store.games.add(game);
    }

    private static LocalDateTime determineFinalDate(Store store, LocalDateTime semifinalDate, int restDays) {
        LocalDateTime minimumFinalDate = semifinalDate.plusDays(restDays).withHour(20).withMinute(0);

        if (store.tournament == null || store.tournament.endDate == null || store.tournament.endDate.trim().isEmpty()) {
            return minimumFinalDate;
        }

        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm");
        try {
            LocalDateTime configuredFinalDate = LocalDateTime.parse(store.tournament.endDate.trim() + " 20:00", formatter);
            if (configuredFinalDate.isBefore(minimumFinalDate)) {
                return minimumFinalDate;
            }
    ...
    private static void createKnockoutGame(Store store, String phase, Equipa teamA, Equipa teamB, LocalDateTime baseDate, int gameIndex) {
        LocalDateTime gameDate = baseDate.withHour(gameIndex % 2 == 0 ? 18 : 20).withMinute(gameIndex % 2 == 0 ? 0 : 30);
        Estadio stadium = store.stadiums.get(gameIndex % store.stadiums.size());
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm");

        Jogo game = new Jogo(
                store.nextId(), phase, teamA.name, teamB.name, gameDate.format(formatter), stadium
        );
        game.state = EstadoJogo.AGENDADO;
        store.games.add(game);
    }

    private static LocalDateTime nextDateAfterPhase(Store store, String phase) {
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm");
        LocalDateTime latest = null;

        for (Jogo game : store.games) {
            if (!phase.equals(game.phase)) continue;
            try {
                LocalDateTime date = LocalDateTime.parse(game.dateTime, formatter);
        ...
        int restDays = store.tournament == null ? 2 : Math.max(1, store.tournament.restDays);
        return latest.plusDays(restDays).withHour(18).withMinute(0);
    }

    private static class ClassificationEntry {
        private final Equipa team;
        private int points;
        private int goalsFor;
        private int goalsAgainst;

        private ClassificationEntry(Equipa team) {
            this.team = team;
        }
